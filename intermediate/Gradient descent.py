...
costs = cost(100, theta1s, pga.distance, pga.accuracy)
...
for m in range(100):
    for n in range(100):
        COST[m,n] = cost(theta0s[m],theta1s[n],pga.distance,pga.accuracy)
...
a = gradient_descent(pga.distance, pga.accuracy,alpha=0.01)
print("Theta1 =", a['theta1'])
plt.scatter(range(0, len(a['costs'])), a['costs'])
plt.show()