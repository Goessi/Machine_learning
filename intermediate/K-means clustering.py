...
def assign_to_cluster(row):
    distance = {}
    keys = list(centroids_dict.keys())
    values = list(centroids_dict.values())
    for i in range(len(centroids_dict)):
        d = calculate_distance(values[i],[row['ppg'],row['atr']])
        distance[i] = d
    min_val = min([v for k,v in distance.items()])
    key = [k for k,v in distance.items() if v == min_val]
    return key[0]
...
def recalculate_centroids(df):
    ...
    for cluster_id in range(0, 4):
        ...
        x = np.average(points['ppg'])
        y = np.average(points['atr'])
        new_centroids_dict[cluster_id] = [x,y]
